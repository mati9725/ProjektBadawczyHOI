...
class VCOCOeval(object):
    ...
    def __init__(self, vsrl_annot_file, coco_annot_file, split_file, threshold):
        """Input:
    ...
    def _do_eval(self, detections_file, ovr_thresh=0.5, transformation_type = None, transformation_level = None):

        self.transformation_type = transformation_type
        self.transformation_level = transformation_level

        vcocodb = self._get_vcocodb()[:1000]
        agent_mAP, agent_TP, agent_FN, agent_FP = self._do_agent_eval(vcocodb, detections_file, ovr_thresh=ovr_thresh)
        # agent_mAP, agent_TP, agent_FN, agent_FP = (0,0,0,0)
        role_mAP, role_TP, role_FN, role_FP = self._do_role_eval(vcocodb, detections_file, ovr_thresh=ovr_thresh, eval_type='scenario_1')
        return agent_mAP, agent_TP, agent_FN, agent_FP, role_mAP, role_TP, role_FN, role_FP
    ...
    def _do_role_eval(self, vcocodb, detections_file, ovr_thresh=0.5, eval_type='scenario_1'):
        ...
        for i in range(len(vcocodb)):
            ...
            if self.transformation_type is 'rotation':
                self.new_h, self.new_w = get_new_shape(vcocodb[i]['height'], vcocodb[i]['width'], self.transformation_level)
            ...
            for aid in range(self.num_actions):
                ...
                for rid in range(len(self.roles[aid]) - 1):
                    ...
                    for j in idx:
                        ...
                        overlaps = self.get_distance(gt_boxes, pred_box, vcocodb[i]['height'], vcocodb[i]['width'])
                        ...
                        if np.all(gt_roles[jmax, :] == -1):  # if no gt role
                            ...
                            ov_role = self.get_distance(gt_roles[jmax, :].reshape((1, 4)),
                                           role_boxes[j, :],
                                           vcocodb[i]['height'],
                                           vcocodb[i]['width'])
                        ...
                        if is_true_action and (ovmax >= ovr_thresh) and (ov_role >= ovr_thresh) and not covered[jmax] and agent_scores[j] > self.threshold:
                            role_TP += 1
        ...
        role_FN = 0
        for aid, elem in enumerate(npos):
            if len(self.roles[aid]) < 2: 
                continue
            else:
                role_FN += int(elem)
        ...
        role_FN = role_FN - role_TP
        role_FP = role_FP - role_TP
        return np.nanmean(role_ap) * 100.00, role_TP, role_FN, role_FP
...
class VCOCOeval(object):
    ...
    def _do_role_eval(self, vcocodb, detections_file, ovr_thresh=0.5, eval_type='scenario_1'):
        ...
        for i in range(len(vcocodb)):
            ...
            if self.transformation_type is 'rotation':test_on_database
                ...
                for j in idx:  # Each predicted person
                    ...
                    overlaps = self.get_distance(gt_boxes,
                                           pred_box,
                                           vcocodb[i]['height'],
                                           vcocodb[i]['width'])  # overlap between this predict human and all human gt_boxes
                    ...
                    if is_true_action and (ovmax >= ovr_thresh) and agent_scores[j] > self.threshold and not covered[jmax]:
                        agent_TP += 1
        ...
        agent_FN = sum(npos) - agent_TP
        agent_FP = agent_FP - agent_TP
        return np.nansum(agent_ap) * 100.00 / self.num_actions, agent_TP, agent_FN, agent_FP

    def get_distance(self, boxes, ref_box, height, width):

        if self.transformation_type is 'rotation':
            ref_box[0],ref_box[1] = rotate_point(ref_box[0],
                                ref_box[1],
                                0.5*width,
                                0.5*height,
                                0.5*self.new_w,
                                0.5*self.new_h, 
                                self.transformation_level)

            ref_box[2],ref_box[3] = rotate_point(ref_box[2],
                                ref_box[3],
                                0.5*width,
                                0.5*height,
                                0.5*self.new_w,
        ...
        elif self.transformation_type is 'scale':
            scale_x = self.transformation_level[0]
        agent_FN = sum(npos) - agent_TP
        agent_FP = agent_FP - agent_TP
        return np.nansum(agent_ap) * 100.00 / self.num_actions, agent_TP, agent_FN, agent_FP
    ...
    def get_distance(self, boxes, ref_box, height, width):
        ...
        if self.transformation_type is 'rotation':
            ...
            scale_x = self.transformation_level[0]
            scale_y = self.transformation_level[1]
            ref_box[0] = ref_box[0]/scale_x
            ref_box[1] = ref_box[1]/scale_y
            ref_box[2] = ref_box[2]/scale_x
            ref_box[3] = ref_box[3]/scale_y
        elif self.transformation_type is 'perspective_transform':
            ref_box[0],ref_box[1] = reverse_perspective(ref_box[0],ref_box[1], width, height, self.transformation_level)
            ref_box[2],ref_box[3] = reverse_perspective(ref_box[2],ref_box[3], width, height, self.transformation_level)
        else:
            return get_overlap(boxes, ref_box)

        centroids_x = 0.5 * (boxes[:, 0] + boxes[:, 2])
        centroids_y = 0.5 * (boxes[:, 1] + boxes[:, 3])
        ref_centroid_x = 0.5 * (ref_box[0] + ref_box[2])
        ref_centroid_y = 0.5 * (ref_box[1] + ref_box[3])

        distances_x = (centroids_x - ref_centroid_x)**2
        distances_y = (centroids_y - ref_centroid_y)**2

        distance = (distances_x + distances_y)**0.5

        for i in range(distance.shape[0]):
            box_diag = ((boxes[i, 0] - boxes[i, 2])**2 + (boxes[i, 1] - boxes[i, 3])**2) ** 0.5
            distance[i] = 1 - distance[i]/box_diag

        return distance

def get_new_shape(height, width, transformation_level):
    image = np.zeros((height, width, 3))
    image = Image.fromarray(np.uint8(image))
    roteted_image = image.rotate(transformation_level, expand=True)
    roteted_image = np.array(roteted_image)
    new_h = roteted_image.shape[0]
    new_w = roteted_image.shape[1]
    return new_h, new_w
...
def reverse_perspective(x, y, width, height, transformation_level):
    transformation_level = transformation_level/100
    x_t_l = x * transformation_level / width

    a = abs(y-0.5*height)
    new_a = a * 0.5/(0.5 - x_t_l)
